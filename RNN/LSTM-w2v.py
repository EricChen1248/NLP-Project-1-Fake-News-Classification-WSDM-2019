...
TRAIN_CSV_PATH = './cutTrainingData.csv'
TEST_CSV_PATH = './cutTestingData.csv'
TRAIN_LABEL_PATH = './label.npy'
W2V_MODEL_PATH = './word2vec_model'
...
def w2v(sentences, word_index):
	...
	wordseq = []
	for i in range(l):
		wordseq.append(text_to_word_sequence(list_sentences[i]))
	pretrain_model = Word2Vec(wordseq, min_count=5)
...
def data_preprocess():
    ...
    # pretrain w2v
    embedding_matrix = w2v(corpus, word_index)
    return train_title1, valid_title1, test_title1, train_title2, valid_title2, test_title2, train_label, valid_label, embedding_matrix, word_index
...
def train():
	...
	#Early Stop
    callbacks = []
    callbacks.append(ModelCheckpoint('./RNN/model-LSTM-w2v/{epoch:05d}-{val_acc:.5f}.h5', monitor='val_acc', save_best_only=False, period=1))
    model.summary()
...
MODEL_PATH = './RNN/model-LSTM-w2v/00003-0.74950.h5'
OUTPUT_FILE_PATH = './RNN/LSTM-w2v.csv'
def predict():
...
train_title1, valid_title1, test_title1, train_title2, valid_title2, test_title2, train_label, valid_label, embedding_matrix, word_index = data_preprocess()
# train()
predict()
