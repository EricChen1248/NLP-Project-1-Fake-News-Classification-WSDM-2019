...
from keras.layers import Embedding, LSTM, concatenate, Dense, GRU
...
def train():
    ...
    # LSTM Layer
    shared_lstm = Bidirectional(GRU(128, return_sequences=True, dropout = DROPOUT))
    top_output = shared_lstm(top_embedded)
    bm_output = shared_lstm(bm_embedded)
    shared_lstm = Bidirectional(GRU(128, return_sequences=True, dropout = DROPOUT))
    top_output = shared_lstm(top_output)
    bm_output = shared_lstm(bm_output)
    shared_lstm = Bidirectional(GRU(64, return_sequences=True, dropout = DROPOUT))
    top_output = shared_lstm(top_output)
    bm_output = shared_lstm(bm_output)
    shared_lstm = Bidirectional(GRU(64, return_sequences=False, dropout = DROPOUT))
    top_output = shared_lstm(top_output)
    bm_output = shared_lstm(bm_output)
    # concatenate
    merged = concatenate([top_output, bm_output], axis=-1)
    output = Dense(64, activation='relu', kernel_regularizer=regularizers.l2(0.1))(merged)
	...
	#Early Stop
    callbacks = []
    callbacks.append(ModelCheckpoint('./RNN/model-GRU-multi-2/{epoch:05d}-{val_acc:.5f}.h5', monitor='val_acc', save_best_only=False, period=1))
    model.summary()
...
MODEL_PATH = './RNN/model-GRU-multi/00013-0.77419.h5'
OUTPUT_FILE_PATH = './RNN/GRUmulti.csv'
def predict():
