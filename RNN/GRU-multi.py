...
def train():
    ...
    # LSTM Layer
    shared_lstm = GRU(128, return_sequences=True, dropout = DROPOUT)
    top_output = shared_lstm(top_embedded)
    bm_output = shared_lstm(bm_embedded)
    shared_lstm = GRU(128, return_sequences=True, dropout = DROPOUT)
    top_output = shared_lstm(top_output)
    bm_output = shared_lstm(bm_output)
    shared_lstm = GRU(64, return_sequences=False, dropout = DROPOUT)
    top_output = shared_lstm(top_output)
    bm_output = shared_lstm(bm_output)
    # concatenate
	...
	#Early Stop
    callbacks = []
    callbacks.append(ModelCheckpoint('./RNN/model-GRU-multi/{epoch:05d}-{val_acc:.5f}.h5', monitor='val_acc', save_best_only=False, period=1))
    model.summary()
